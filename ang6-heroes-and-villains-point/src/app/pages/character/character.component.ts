...
export class CharacterComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscription.push(this.lazyLoadComics.searchData.subscribe(returnQuery => {
      this.getComics();
    }));
    this.subscription.push(this.lazyLoadSeries.searchData.subscribe(returnQuery => {
      this.getSeries();
    }));
    this.subscription.push(this.lazyLoadStories.searchData.subscribe(returnQuery => {
      this.getStories();
    }));
    ...
    this.subscription.push(this.route.params.subscribe(routeParam => {
      this.nameCharacter = routeParam.name;
      this.idCharacter = routeParam.id;
      this.path = 'character/' + encodeURI(this.nameCharacter) + '/' + this.idCharacter + '#header';
      this.path = this.path.replace('(', '%28');
      this.path = this.path.replace(')', '%29');
    }));
  ...
  marcarFavorito(valor: boolean, object: any) {
    ...
    this.subscription.push(snackBarRef.onAction().subscribe(() => object.favorite = !object.favorite));
  }
